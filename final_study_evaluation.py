...
#function for percentage of agreement on level using average of the difference between levels
def agreed_level_score(diff):
    if diff == 0:  # perfect agreement
        agreement_score_level = 1.0
    elif 0<diff <= 1:  #high agreement
        agreement_score_level = 0.75
    elif 1<diff <= 2:  # moderate agreement
        agreement_score_level = 0.50
    elif 2<diff<=3:  # low agreement
...
for picture in unique_images:
    ...
    if len(rows_for_picture) == 3:
        ...
        if len(unique_cat_pic) == 1:
            ...
            level_values = rows_for_picture[rows_for_picture['Category'] == agreed_category]['Level']
            ...
            print(differences)
            avg_diff=round(differences.mean(),2)
            print(avg_diff)
            ...
            if (unique_count == max_count).sum() == 1:
                ...
                if not filtered_rows.empty:  
                    #taking average of the levels
                    avg_level = filtered_rows[filtered_rows['Category'] == agreed_category]['Level'].mean()
                    agreed_level=avg_level.round(2)
                    level_diff = filtered_rows[filtered_rows['Category'] == agreed_category]['Level'].diff().abs().dropna()
                    print(f"level_diff{agreed_level}")
                    level_diff_round=np.mean(level_diff)
                    agreement_score_level=agreed_level_score(level_diff_round) 
                    print(agreement_score_level)
            else:
                print(f"Image: {picture}")
...
def category_agreement():
    ...
    sns.stripplot(x='agreed_category', y='level_average', data=drop_no_errors, jitter=True)
    plt.title('Scatter Plot of Levels per Category',fontsize=16)
    plt.xlabel('Category',fontsize=14)
    plt.ylabel('Level Average',fontsize=14)
